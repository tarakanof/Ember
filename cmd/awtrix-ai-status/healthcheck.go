...
// container's loopback. When STATUS_TLS_CERT_FILE is set, the default
...
//   - STATUS_HEALTHCHECK_CA_FILE: PEM bundle to add to the trust pool.
//   - STATUS_HEALTHCHECK_INSECURE=1|true: skip verify. Fine on a trusted LAN
//     where the operator can't be bothered to mount a CA bundle.
func healthcheckOnce(url string) error {
	client := &http.Client{Timeout: 2 * time.Second}
	if strings.HasPrefix(url, "https://") {
		tlsCfg := &tls.Config{}
		if caFile := os.Getenv("STATUS_HEALTHCHECK_CA_FILE"); caFile != "" {
