...
func TestUninstallSettings_RestoresStatusLine(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, ".config", "ember"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(sb), "mine.sh") || strings.Contains(string(sb), "ember-claude-producer statusline") {
...
func TestUninstall_StripsHooksAndRemovesPlist(t *testing.T) {
	...
	plistP := filepath.Join(plistDir, "com.ember.heartbeat.plist")
	...
	if strings.Contains(string(body), "ember-claude-producer") {
