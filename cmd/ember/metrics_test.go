...
func TestRender_StructureAndHelpLines(t *testing.T) {
	...
	for _, name := range []string{
		...
		"ember_publish_total",
		"ember_rate_limit_denied_total",
		"ember_sessions_evicted_total",
		"ember_sessions_active",
		"ember_uptime_seconds",
		"ember_last_publish_unix",
		"ember_last_publish_ok",
	...
	if !strings.Contains(body, `ember_requests_total{pattern="POST /v1/status",status="200"} 2`) {
		t.Errorf("body missing the 200-counter line:\n%s", body)
	}
	if !strings.Contains(body, `ember_requests_total{pattern="POST /v1/status",status="400"} 1`) {
		t.Errorf("body missing the 400-counter line:\n%s", body)
	}
	if !strings.Contains(body, `ember_publish_total{result="ok"} 1`) {
		t.Errorf("body missing publish ok line:\n%s", body)
	}
	if !strings.Contains(body, `ember_publish_total{result="fail"} 1`) {
		t.Errorf("body missing publish fail line:\n%s", body)
	}
	if !strings.Contains(body, `ember_rate_limit_denied_total 1`) {
		t.Errorf("body missing rate_limit_denied line:\n%s", body)
	}
	if !strings.Contains(body, `ember_sessions_evicted_total 1`) {
...
func TestRender_GaugesReadFromApp(t *testing.T) {
	...
	app.metrics.render(&buf, app)
	body := buf.String()
	if !strings.Contains(body, "ember_sessions_active 2") {
		t.Errorf("sessions_active wrong:\n%s", body)
	}
	if !strings.Contains(body, "ember_last_publish_ok 1") {
		t.Errorf("last_publish_ok wrong:\n%s", body)
	}
	if !strings.Contains(body, "ember_last_publish_unix 1700000000") {
		t.Errorf("last_publish_unix wrong:\n%s", body)
	}
	if !strings.Contains(body, "ember_ratelimit_buckets 1") {
		t.Errorf("ratelimit_buckets wrong:\n%s", body)
	}
	upRe := regexp.MustCompile(`ember_uptime_seconds (\d+\.\d+)`)
	if !upRe.MatchString(body) {
		t.Errorf("uptime_seconds gauge missing:\n%s", body)
	}
	if !strings.Contains(body, `ember_build_info{revision=`) {
...
func TestRender_LastPublishUnixZeroWhenNoPublish(t *testing.T) {
	...
	var buf bytes.Buffer
	app.metrics.render(&buf, app)
	if !strings.Contains(buf.String(), "ember_last_publish_unix 0") {
...
func TestMetricsEndpoint_PublicAndPlainText(t *testing.T) {
	...
	if !strings.Contains(string(body), "ember_build_info{") {
		t.Errorf("body missing ember_build_info; got:\n%s", body)
	}
}
...
func TestRoutes_RequestCountersWireUpEndToEnd(t *testing.T) {
	...
	defer resp.Body.Close()
	body, _ := io.ReadAll(resp.Body)
	wantLine := `ember_requests_total{pattern="GET /healthz",status="200"} 4`
