...
const launchAgentLabel = "com.awtrix-ai-status.codex"
...
func runInstall() {
	...
	fmt.Println("Ensure ~/.config/awtrix-ai-status/producer.env has STATUS_SOURCE + STATUS_SERVER_URL + STATUS_TOKEN.")
}
...
func generatePlist(binPath, home string) []byte {
	logPath := filepath.Join(home, "Library", "Logs", "awtrix-codex-producer.log")
...
func envExample() string {
	return `# awtrix-ai-status producer configuration (shared by Claude + Codex producers)
...
# STATUS_CODEX_POLL_INTERVAL_MS=2000
# STATUS_CODEX_ACTIVITY_WINDOW_SECONDS=300
# STATUS_CODEX_SESSIONS_DIR=~/.codex/sessions
`
}
