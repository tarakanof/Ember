...
func TestComposeFrameSourceCard(t *testing.T) {
	...
	f := ComposeFrame(s, cardSource, colorForState(s.State), []Session{s}, time.Now())
...
func TestComposeFrameNoCardBlankNumberSlot(t *testing.T) {
	...
	f := ComposeFrame(s, -1, colorForState(s.State), []Session{s}, time.Now())
	for y := 1; y <= 5; y++ {
...
func TestComposeFrameBottomBarModes(t *testing.T) {
	...
	f = ComposeFrame(off, cardSource, colorForState(off.State), []Session{off}, time.Now())
	for x := 0; x < 32; x++ {
		if f.Dirty[barRow][x] {
	...
	f = ComposeFrame(rate, cardSource, colorForState(rate.State), []Session{rate}, time.Now())
	if !f.Dirty[barRow][8] {
